...
From your build environment, execute the `build` command. If it fails for some (non-obvious) reason, open an issue on [this repository](https://github.com/seeq12/directory-watch-connector/issues) with the build output and error message.
...
## Installation
1. Extract the generated Install Kit _*.zip_ from the [package](https://github.com/seeq12/directory-watch-connector/tree/main?tab=readme-ov-file#packaging-your-connector) step above to a directory of your choice.
2. Shut down Seeq.
3. If you already have DirectoryWatch installed, make a backup of `$DATA\data\plugins\connectors\DirectoryWatch`. Kindly ensure that the backup is in a directory **outside the `$DATA\data\plugins\connectors` directory** otherwise the Seeq Agent may accidentally load the backup instead of the updated connector, where `$DATA` is your Seeq data folder. The Seeq data folder is `C:\ProgramData\Seeq` by default.  If the DirectoryWatch folder doesn't exist, you'll need to create it first.
...
5. If you already have a copy of DirectoryWatch installed, make a backup of your current `$DATA\data\configuration\link\DirectoryWatch Connector.json`.
...
6. No other modifications should be needed to the config files.  After saving the changes to the configurations, go back to `$DATA\data\configuration\link\DirectoryWatch Connector.json`, make a small change to the whitespace in the file, and save.  This will prompt a restart of the connector so that the changes to the config files get picked up.
7. After the connector restarts, it should begin watching directories and reading files that pass the filters. When a file is opened for reading, the file extension is changed to `.importing`. If the read operation succeeds, the file extension will change again, this time to `.imported`; otherwise, the extension will remain `.importing`.
...
11. In the `DataFileReaders` folder of the Install Kit, there is a directory for each Reader that comes with the installation.  Within the root folder for each Reader, if special steps are necessary, there will be a `README.md` file that describes further configuration steps for the reader.
...
Next, update the `clean.bat` script to include your Reader in the clean up process. Update the `build.bat` script to allow for installing your Reader's dependencies. Finally, update the `package.ps1` powershell script, updating the `$READER_DIRS` variable with your Reader to include it in the built Install Kit package.
